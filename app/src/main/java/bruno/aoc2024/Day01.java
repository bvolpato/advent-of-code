 ...
 *
 * <p>The Chief Historian is always present for the big Christmas sleigh launch, but nobody has seen
 ...
 * <p>For example:
 *
 * <p>3 4 4 3 2 5 1 3 3 9 3 3
 *
 * <p>Maybe the lists are only off by a small amount! To find out, pair up the numbers and measure
 * how far apart they are. Pair up the smallest number in the left list with the smallest number in
 * the right list, then the second-smallest left number with the second-smallest right number, and
 * so on.
 *
 ...
 *
 * <p>To find the total distance between the left list and the right list, add up the distances
...
public class Day01 {
   ...
   * --- Part Two ---
   *
   * <p>Your analysis only confirmed what everyone feared: the two lists of location IDs are indeed
   * very different.
   *
   * <p>Or are they?
   *
   * <p>The Historians can't agree on which group made the mistakes or how to read most of the
   ...
   *
   * <p>This time, you'll need to figure out exactly how often each number from the left list
   ...
   * <p>3 4 4 3 2 5 1 3 3 9 3 3
   *
   * <p>For these example lists, here is the process of finding the similarity score:
   *
   ...
   * the right list once, so the similarity score increases by 4 * 1 = 4. The third number in the
   * left list is 2. It does not appear in the right list, so the similarity score does not increase
   * (2 * 0 = 0). The fourth number, 1, also does not appear in the right list. The fifth number, 3,
   * appears in the right list three times; the similarity score increases by 9. The last number, 3,
   * appears in the right list three times; the similarity score again increases by 9.
   *
   * <p>So, for these example lists, the similarity score at the end of this process is 31 (9 + 4 +
   * 0 + 0 + 9 + 9).
   *
  ...
  public static void part2(String input) {
    ...
    for (int leftNum : left) {
      long count = right.stream().filter(n -> n == leftNum).count();
