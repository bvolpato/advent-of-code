...
public class Day11 {
    ...
    public static void main(String[] args) throws IOException, UnsupportedFlavorException {
        ...
        TreeSet<Integer> rowWithoutGalaxies = new TreeSet<>(IntStream.rangeClosed(0, grid.length).mapToObj(i -> i).collect(Collectors.toList()));
        rowWithoutGalaxies.removeAll(rowWithGalaxies);

        TreeSet<Integer> colWithoutGalaxies = new TreeSet<>(IntStream.rangeClosed(0, grid[0].length).mapToObj(i -> i).collect(Collectors.toList()));
        ...
        for (int i = 0; i < galaxies.size(); i++) {
            for (int j = i + 1; j < galaxies.size(); j++) {
                ...
                if (galaxies.get(i)[0] <= galaxies.get(j)[0]) {
                    noGalaxyRowsInBetween = rowWithoutGalaxies.subSet(galaxies.get(i)[0], false, galaxies.get(j)[0], false).size();
                } else {
                    noGalaxyRowsInBetween = rowWithoutGalaxies.subSet(galaxies.get(j)[0], false, galaxies.get(i)[0], false).size();
                }
                int noGalaxyColsInBetween;
                if (galaxies.get(i)[1] <= galaxies.get(j)[1]) {
                    noGalaxyColsInBetween = colWithoutGalaxies.subSet(galaxies.get(i)[1], false, galaxies.get(j)[1], false).size();
                } else {
                    noGalaxyColsInBetween = colWithoutGalaxies.subSet(galaxies.get(j)[1], false, galaxies.get(i)[1], false).size();
                }

                System.out.println("Between (" + galaxies.get(i)[0] + "," + galaxies.get(i)[1] + ") and (" + galaxies.get(j)[0] + "," + galaxies.get(j)[1] + ") there are empty: " + noGalaxyRowsInBetween + " rows and " + noGalaxyColsInBetween + " cols.");
