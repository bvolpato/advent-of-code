...
public class Day12 {
  ...
  public static void main(String[] args) throws IOException {
    String clipboard = readDay(2023, 12);
    System.out.println("Clipboard content: " + clipboard);

    List<String> contentLines = AOCReader.lines(clipboard);
    ...
    for (int i = 0; i < grid.length; i++) {
      ...
      System.out.println(
          new String(grid[i]) + " " + Arrays.toString(groupSizes.get(i)) + " == " + ans);
      sumAll += ans;
    }
  ...
  public static long dp(
      char[] line, int index, int currGroup, int[] groups, int groupIndex, Map<Long, Long> memo) {
